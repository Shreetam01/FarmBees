...
const VerifiedExpert = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8080/admin/get-approved-exp`, {
                    method: "GET",
                    headers: {
                ...
                {verifiedExp.length > 0 ? (
                    verifiedExp.map((item, index) => (
                        ...
                        <React.Fragment key={index}>
                            <ExpertCart id={item.id} fullName={item.fullName} email={item.email} qualification={item.qualification} />
                            <hr />
                        </React.Fragment>
                    ))
                ) : (
